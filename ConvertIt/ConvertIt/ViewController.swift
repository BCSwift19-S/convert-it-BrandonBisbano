...
//  ConvertIt
//
//  Created by Brandon Bisbano on 2/24/19.
...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var negativeSegment: UISegmentedControl!
    ...
    @IBAction func userInputChanged(_ sender: UITextField) {
        ...
        if userInput.text?.first == "-" {
            negativeSegment.selectedSegmentIndex = 1
        } else {
            negativeSegment.selectedSegmentIndex = 0
        }
    }
    ...
    @IBAction func negativeSelected(_ sender: UISegmentedControl) {
        if negativeSegment.selectedSegmentIndex == 0 { // + selected, remove any -
...
extension ViewController: UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        if conversionString.lowercased().contains("celsius") {
            negativeSegment.isHidden = false
        } else {
            negativeSegment.isHidden = true
            userInput.text = userInput.text?.replacingOccurrences(of: "-", with: "")
            negativeSegment.selectedSegmentIndex = 0
